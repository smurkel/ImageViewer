...
class Texture:
    ...
    def __init__(self, format = "ru16", interp = False):
    ...
    def update(self, pixeldata, width = None, height = None):
        ...
        if pixeldata is None:
            ...
            if self.format == "ru16" or self.format == "rgbu16":
                imgdata = pixeldata.flatten().astype(np.uint16)
            else:
                imgdata = pixeldata.flatten().astype(np.float)

            glTexImage2D(GL_TEXTURE_2D, 0, self.internalformat, self.width, self.height, 0, self.format, self.type, imgdata)

    def setPixel(self, pixelCoords, pixelColor):
        self.bind()
        x = int(pixelCoords[0])
        y = int(pixelCoords[1])
        pixelColor = np.array([pixelColor[0], pixelColor[1], pixelColor[2], 1.0])
        pixelColor = pixelColor.flatten().astype(np.float)
        glTexSubImage2D(GL_TEXTURE_2D, 0, x, y, 1, 1, self.format, self.type, pixelColor)
